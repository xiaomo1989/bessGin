...
func UserList(wheres map[string]interface{}) (list []model.User) {
	...
	util.DB().Debug().Where(wheres).Unscoped().Order("id desc").Find(&users)
	return users
}
...
func UserPage(wheres map[string]interface{}, pageIndex int, pageSize int) (list []model.User) {
	...
	util.DB().Debug().Where(wheres).Unscoped().Order("id desc").Limit(pageSize).Offset(offset).Find(&list)
	return list
}
...
func UserTotal(wheres map[string]interface{}) int64 {
	var user []model.User
	var total int64
	util.DB().Debug().Where(wheres).Find(&user).Count(&total)
	return total
}
